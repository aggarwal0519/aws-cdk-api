...
export const getAddressHandler = async (event: any) => {
    ...
    if (suburb) {
        params.FilterExpression += ' AND contains(address.suburb, :suburb)';
    }
    
    if (postcode) {
        params.FilterExpression += ' AND contains(address.postcode, :postcode)';
    ...
    try {
        ...
        const unmarshalledItem = Item? unmarshall(Item) : {};
